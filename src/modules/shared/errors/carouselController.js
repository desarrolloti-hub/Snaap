export const carouselController = {
    render(container) {
        ...
        section.innerHTML = `
            ...
            <button class="nav-button btn-prev" id="snaap-prev-btn"><svg viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"></polyline></svg></button>
            <button class="nav-button btn-next" id="snaap-next-btn"><svg viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"></polyline></svg></button>
        `;
        ...
        const renderCards = () => {
            ...
            cardData.forEach((item, index) => {
                ...
                card.className = 'card';
                if (index === currentActiveIndex) card.classList.add('active');
                else if (index === (currentActiveIndex - 1 + cardData.length) % cardData.length) card.classList.add('prev');
                else if (index === (currentActiveIndex + 1) % cardData.length) card.classList.add('next');
                else card.classList.add('hidden');
                card.innerHTML = `<h3>${item.title}</h3><img src="${item.img}" alt="${item.title}"><div class="price">${item.price}</div>`;
        ...
        renderCards();
        section.querySelector('#snaap-prev-btn').addEventListener('click', () => move(-1));
        section.querySelector('#snaap-next-btn').addEventListener('click', () => move(1));
